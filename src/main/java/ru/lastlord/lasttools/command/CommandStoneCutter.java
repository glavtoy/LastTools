...
public class CommandStoneCutter implements CommandExecutor {
    ...
    public CommandStoneCutter() {
        LastTools.instance.getCommand("stonecutter").setExecutor(this);
    }
    ...
    public boolean onCommand(@NotNull CommandSender sender, @NotNull Command command, @NotNull String label, @NotNull String[] args) {
        if (sender instanceof Player) {
            ...
            if (player.hasPermission("lasttools.stonecutter") || player.hasPermission("*") || player.isOp()) {
                player.openStonecutter(player.getLocation(), true);
            } else {
                for (Rank rank : RankManager.ranks) {
                    if (rank.getPermissions().contains("lasttools.stonecutter") || rank.getPermissions().contains("*")) {
