...
public class CommandSmithingTable implements CommandExecutor {

    public CommandSmithingTable() {
        LastTools.instance.getCommand("smithingtable").setExecutor(this);
    }
    ...
    public boolean onCommand(@NotNull CommandSender sender, @NotNull Command command, @NotNull String label, @NotNull String[] args) {
        if (sender instanceof Player) {
            ...
            if (player.hasPermission("lasttools.smithingtable") || player.hasPermission("*") || player.isOp()) {
                player.openSmithingTable(player.getLocation(), true);
            } else {
                for (Rank rank : RankManager.ranks) {
                    if (rank.getPermissions().contains("lasttools.smithingtable") || rank.getPermissions().contains("*")) {
